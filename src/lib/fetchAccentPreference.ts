...
/** Default accent — hyper green (CTAs / orb); shell chrome stays charcoal via CSS. */
export const FETCH_ACCENT_DEFAULT_HEX = '#00ff6a'
...
/** Applies accent to the live document (buttons, orb, etc.). Does not repaint app shell — that stays charcoal in CSS. */
export function applyFetchAccentToRoot(hex: string) {
  ...
  root.style.removeProperty('--fetch-bottom-nav-chrome')
