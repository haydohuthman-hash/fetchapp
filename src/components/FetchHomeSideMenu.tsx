...
const ICON = 'h-5 w-5 shrink-0 text-red-700'
...
function previewGradient(style: HardwareProduct['previewStyle']) {
  switch (style) {
    ...
    case 'blue':
      return 'from-red-600/35 via-red-500/25 to-transparent'
    default:
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
        ...
        <button
          type="button"
          className="fetch-home-map-side-menu-dismiss rounded-full px-3 py-1.5 text-[13px] font-semibold text-red-700 transition-colors hover:bg-red-50"
          onClick={onClose}
        >
          Close
        </button>
      ...
      <nav className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden p-3 pb-2">
        ...
        {onAlerts ? (
          ...
          >
            ...
            {alertsUnreadCount > 0 ? (
              <span className="ml-auto inline-flex min-w-[1.25rem] justify-center rounded-full bg-fetch-red px-1 text-[10px] font-bold text-white">
      ...
      >
        <p className="mb-2 px-1 text-[10px] font-bold uppercase tracking-[0.2em] text-red-700">
          Wall panels
        </p>
        <div
          className={[
          ...
          {products.map((p) => (
            ...
            >
              ...
              <div className="px-3 py-2.5">
                ...
                <p className="mt-1 text-[10px] font-medium uppercase tracking-[0.14em] text-red-700">
                  View
                </p>
              </div>
            </button>
