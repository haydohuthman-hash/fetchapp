...
const headerChromeIconBtn =
  'fetch-header-chrome-icon-btn fetch-apple-warp-btn flex h-10 w-10 shrink-0 items-center justify-center rounded-md border-0 bg-transparent p-0 text-zinc-800 shadow-none ring-0 transition-[transform,opacity] hover:opacity-75 active:scale-[0.96]'
...
export function FetchHomeAppAddressHeader({
  ...
  return (
    <header
      className={[
        'fetch-app-address-header pointer-events-auto fixed left-0 right-0 top-0 z-[56] border-b border-zinc-200/90 bg-white',
        chromeShellSlide,
        'translate-y-0',
      ].join(' ')}
      ...
      >
        <div className="flex w-full min-w-0 items-center gap-2">
          <button
            ...
            className="fetch-header-search-bar fetch-apple-warp-btn flex min-h-[2.5rem] min-w-0 flex-1 items-center gap-2 rounded-full border border-zinc-200/95 bg-zinc-50 py-0 pl-3 pr-3 text-left transition-[border-color,opacity,transform] hover:border-zinc-300 active:scale-[0.99]"
            aria-label={`Search — ${searchHint}`}
          >
            <SearchGlyph className="shrink-0 text-zinc-500" />
            <span
              key={hintIdx}
              className="fetch-header-search-hint min-w-0 truncate text-[14px] font-medium text-zinc-500"
            >
              {searchHint}
            </span>
          </button>
