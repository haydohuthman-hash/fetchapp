...
/** Single promo badge on the hero (shared across slides). */
const HOME_TOP_BANNER_BADGE_LABEL = 'LIVE NOW'
...
function portraitUrlForLocalSellerKey(key: string): string {
  ...
  return LOCAL_SELLER_PORTRAIT_URLS[h % LOCAL_SELLER_PORTRAIT_URLS.length]
}
...
function HomeTopBannerSlideshow({ onOpenDrops }: { onOpenDrops: () => void }) {
  ...
  const onChevronNext = useCallback(
  ...
  return (
    <section
      className="-mx-0.5 px-0.5 mt-[3.875rem]"
    ...
    >
      <div
        ...
        className="relative w-full cursor-grab overflow-hidden rounded-2xl bg-black shadow-none ring-0 touch-pan-y active:cursor-grabbing"
        onPointerDown={onBannerPointerDown}
        onPointerMove={onBannerPointerMove}
        onPointerUp={onBannerPointerUp}
        onPointerCancel={onBannerPointerCancel}
      >
        <div ref={viewportRef} className="relative aspect-[16/9] w-full overflow-hidden">
          <div
            className={[
          ...
          {/* Dark fade at the bottom of the banner. */}
          <div
            className="pointer-events-none absolute inset-x-0 bottom-0 z-[3] h-[75%] bg-[linear-gradient(to_top,rgba(0,0,0,0.99)_0%,rgba(0,0,0,0.96)_42%,rgba(0,0,0,0.78)_60%,rgba(0,0,0,0.22)_82%,transparent_100%)]"
            aria-hidden
          />
          {/* Slight overall sheen to keep text legible against bright images. */}
          <div
            className="pointer-events-none absolute inset-0 z-[3] bg-gradient-to-t from-black/35 via-transparent to-transparent opacity-70"
            aria-hidden
          />

          <div
            className="pointer-events-none absolute left-3 top-3 z-[5] sm:left-3.5 sm:top-3.5"
            aria-hidden
          >
            <span className="inline-flex items-center rounded-full bg-[#ffffff] px-2.5 py-1 text-[10px] font-extrabold uppercase leading-none tracking-[0.12em] text-[#000000] shadow-[0_2px_14px_rgba(0,0,0,0.35)] ring-1 ring-black/10 sm:px-3 sm:py-1.5 sm:text-[11px]">
              {HOME_TOP_BANNER_BADGE_LABEL}
            </span>
          </div>

          {safeSlides[index]?.headline ? (
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[4] flex min-w-0 flex-col justify-end gap-1 px-3 pb-3.5 pr-[3.75rem] pt-12 sm:px-3.5 sm:pb-4 sm:pr-16">
              <p className="min-w-0 max-w-full overflow-hidden text-ellipsis whitespace-nowrap text-left text-[0.9375rem] font-black uppercase leading-none tracking-[0.035em] text-white [text-shadow:0_2px_24px_rgba(0,0,0,0.92),0_1px_2px_rgba(0,0,0,0.6)] sm:text-lg sm:tracking-[0.03em]">
                {safeSlides[index]!.headline}
              </p>
              {safeSlides[index]!.subtext ? (
                <p className="min-w-0 max-w-full overflow-hidden text-ellipsis whitespace-nowrap text-left text-[11px] font-semibold leading-none tracking-[-0.01em] text-white/85 [text-shadow:0_1px_14px_rgba(0,0,0,0.85)] sm:text-xs">
                  {safeSlides[index]!.subtext}
                </p>
              ) : null}
            </div>
          ...
          {slideCount > 1 ? (
            <button
              type="button"
              onClick={onChevronNext}
              onPointerDown={(e) => e.stopPropagation()}
              className="absolute bottom-3 right-3 z-[6] flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/45 text-white shadow-[0_4px_24px_rgba(0,0,0,0.45)] backdrop-blur-[6px] transition-[transform,background-color] active:scale-[0.94] motion-reduce:transition-none"
      ...
      {slideCount > 1 ? (
        <div
          className="fetch-home-banner-dots relative z-[6] flex justify-center gap-2 bg-transparent px-2 py-2.5"
          role="tablist"
...
function ExplorePeerListingCard({
  ...
  return (
    <div className="flex min-w-0 flex-col overflow-hidden rounded-lg bg-[#1a1b1f] text-left text-zinc-200 shadow-none ring-0">
      <div className="relative aspect-square w-full bg-[#15161a]">
        {img ? (
          <img
        ...
        {showQuickAdd ? (
          <button
            ...
            className={[
              'absolute right-1 top-1 z-[3] flex h-8 w-8 items-center justify-center overflow-hidden rounded-full shadow-sm shadow-black/25 transition-transform',
      ...
      <button
        type="button"
        aria-label={label}
        className="flex min-w-0 flex-col gap-0.5 px-2 pb-2 pt-2 text-left outline-none active:bg-white/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#00ff6a]"
        onClick={() => onOpenPeerListing(l.id)}
      >
        <div className="flex min-w-0 items-baseline gap-1">
          <p className="min-w-0 truncate text-[13px] font-bold tabular-nums text-white">
            {priceStr}
          </p>
          <span className="whitespace-nowrap text-[11px] font-bold text-zinc-400">
            {fulfillment}
          </span>
        </div>
        <p className="flex items-center gap-1 text-left text-[10px] font-bold text-zinc-300">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" aria-hidden className="shrink-0 text-amber-400">
            <path d="M13 2L4.5 14H12l-1 8 8.5-12H12l1-8z" fill="currentColor" />
          </svg>
          <span>or fetch it from {fetchFromPrice}</span>
        </p>
        <p className="line-clamp-2 text-left text-[11px] font-semibold leading-snug text-white">
          {l.title}
        </p>
      </button>
    </div>
...
function ForYouLocalSellerCarouselCard({
  ...
  return (
    <button
      ...
      className="flex w-[calc((100%-1rem)/3.1)] shrink-0 flex-col bg-transparent p-0 text-left transition-transform active:scale-[0.98]"
      aria-label={`${handle}, ${seller.listingCount} active listings — open in marketplace`}
    >
      <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-zinc-100 dark:bg-zinc-800">
        <img
          src={portraitSrc}
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[2] flex min-w-0 flex-col gap-y-1 px-1.5 pb-1.5 pt-6">
          <div className="flex min-w-0 leading-none">
            <p className="min-w-0 truncate text-[12px] font-bold text-white [text-shadow:0_1px_2px_rgba(0,0,0,0.65)]">
              {handle}
            </p>
          </div>
          <p className="flex min-w-0 items-center gap-0.5 text-left text-[9px] font-bold leading-none text-white [text-shadow:0_1px_2px_rgba(0,0,0,0.55)]">
            ...
            <span className="min-w-0 truncate">{seller.listingCount} live listings · shop all</span>
          </p>
        </div>
      </div>
    </button>
...
function ForYouPremiumFeedTabs({
  ...
  return (
    <div
      className="mt-2 w-full min-w-0 self-stretch px-3 sm:px-4"
      role="tablist"
      aria-label="Feed"
    >
      <div
        className={[
          ...
          'bg-[rgba(0,0,0,0.92)]',
          'shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_8px_28px_-12px_rgba(0,0,0,0.9)]',
          'backdrop-blur-xl backdrop-saturate-[1.2]',
          '[-webkit-backdrop-filter:blur(20px)_saturate(1.2)]',
        ].join(' ')}
      >
        <div
                ...
                className={[
                  ...
                  selected
                    ? [
                        ...
                        'bg-white/[0.14]',
                        'shadow-[0_0_0_1px_rgba(255,255,255,0.12),inset_0_1px_0_rgba(255,255,255,0.28),0_0_28px_-4px_rgba(255,255,255,0.35),0_6px_20px_-8px_rgba(255,255,255,0.12)]',
                      ].join(' ')
                    : [
...
function HomeShellForYouFeedInner({
  ...
  const featuredListingsNearYou = useMemo(() => allListings.slice(0, 8), [allListings])

  const topLocalSellers = useMemo(() => localSellersFromPeerListings(allListings), [allListings])
    ...
    return {
        ...
        <section className="mb-1 min-w-0" aria-labelledby="fetch-embed-live-feed-heading">
          <div className="mb-0 flex items-center justify-between gap-1.5 px-0.5">
            ...
            <button
              type="button"
              onClick={onOpenDrops}
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-transparent text-white/80 transition-colors hover:bg-transparent hover:text-white active:scale-[0.96]"
      ...
      sellers: (
        <section className="mb-1 min-w-0" aria-labelledby="fetch-top-local-sellers-heading">
          <div className="mb-0 flex items-center justify-between gap-1.5 px-0.5">
            <h3
              id="fetch-top-local-sellers-heading"
              className="text-[12px] font-bold leading-none uppercase tracking-[0.06em] text-zinc-600"
            >
              Top sellers
            </h3>
          ...
          <div className="-mx-0.5 flex gap-2 overflow-x-auto overflow-y-hidden pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [-webkit-overflow-scrolling:touch] px-2 [&::-webkit-scrollbar]:hidden">
            {topLocalSellers.map((s) => (
      ...
      featured: (
        <section className="mb-1 min-w-0" aria-labelledby="fetch-featured-near-you-heading">
          <div className="mb-0 flex items-center justify-between gap-1.5 px-0.5">
            <h3
              id="fetch-featured-near-you-heading"
              className="text-[12px] font-bold leading-none uppercase tracking-[0.06em] text-zinc-600"
            >
              Listings {scopeLabel}
            </h3>
      ...
      grid: (
        <section className="mb-1 min-w-0" aria-labelledby="fetch-explore-grid-heading">
          <h3
            ...
            className="mb-2 px-0.5 text-[12px] font-bold leading-none uppercase tracking-[0.06em] text-zinc-600"
          >
            Browse all
          </h3>
  ...
  if (embedded) {
    return (
      ...
      >
        <HomeTopBannerSlideshow onOpenDrops={onOpenDrops} />
