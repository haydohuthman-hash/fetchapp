...
const SLIDER_THEME = {
  ...
  /** Buy: black copy + chevrons on white fill. */
  buy: {
    label: '#000000',
    fill: '#ffffff',
    border: 'rgba(255, 255, 255, 0.42)',
    chevron: ['rgba(0, 0, 0, 0.38)', 'rgba(0, 0, 0, 0.58)', 'rgba(0, 0, 0, 0.88)'] as const,
  },
} as const
...
const DENSITY = {
  ...
  /** Drops reel overlay: shorter pill for right-rail layout. */
  reels: { shellPad: 2, trackH: 34, borderW: 2, labelPx: 11, chevronSvgH: 17 },
} as const

/** Outer height (px) of the pill shell — align sibling controls (e.g. Custom) to this. */
export function fetchCommercePillShellHeightPx(density: keyof typeof DENSITY): number {
  const d = DENSITY[density]
  return d.shellPad * 2 + d.borderW * 2 + d.trackH
}
